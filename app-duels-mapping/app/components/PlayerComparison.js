...
export default function PlayerComparison({
 ...
 const centerValuesPlugin = {
  ...
  beforeDraw(chart, _args, _pluginOptions) {
    ...
    const leftColor = Array.isArray(ds?.backgroundColor) ? ds.backgroundColor[0] : "#A1D17E";
    const rightColor = Array.isArray(ds?.backgroundColor) ? ds.backgroundColor[1] : "#3B5B84";
  ...
  return (
    <Box key={stat} sx={{ textAlign: "center", width: 160, height: 160 }}>
      <Doughnut
        ...
        key={`${stat}-${playerA?.player_name ?? "na"}-${playerB?.player_name ?? "nb"}-${theme.palette.mode}`}
        data={data}
        options={options}
  ...
  return (
    <Container maxWidth="md">
      <Box sx={{ py: 3 }}>
        ...
        {/* Season Buttons */}
        {/* Year Selector */}
        <Grid container alignItems={"center"} justifyContent={"center"} mb={8}>
          <Grid
            item
            size={{ xs: 12, md: 10 }}
